  ...
  //日志的起始索引，默认为 -1
   ...
   //已提交的日志索引
    ...
    public DLedgerEntry appendAsLeader(DLedgerEntry entry) {
        System.out.println("【Leader 节点存储数据】");
        //Leader 节点存储数据
        PreConditions.check(memberState.isLeader(), DLedgerResponseCode.NOT_LEADER);
        //当前磁盘是否已满，其判断依据是 DLedger 的根目录或数据文件目录的使用率超过了允许使用的最大值，默认值为85%
        PreConditions.check(!isDiskFull, DLedgerResponseCode.DISK_FULL);

        //从本地线程变量获取一个数据与索引 buffer。其中用于存储数据的 ByteBuffer，其容量固定为 4M ，
        // 索引的 ByteBuffer 为两个索引条目的长度，固定为64个字节
...
//构建条目索引并将索引数据追加到 pagecache
    ...
    public void updateCommittedIndex(long term, long newCommittedIndex) {
        ...
        //更新 commitedIndex、committedPos 两个指针，DledgerStore会定时将已提交指针刷入 checkpoint 文件，达到持久化 commitedIndex 指针的目的
