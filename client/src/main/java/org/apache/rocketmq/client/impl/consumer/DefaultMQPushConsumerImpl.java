...
/**
 * 默认的消费者 这个作用最大 可以认为是整个消费流程的协调者(消费拉取和消息消费)
 */
public class DefaultMQPushConsumerImpl implements MQConsumerInner {
    ...
    /**
     * 消息消费逻辑  内部会维护一个线程池  注意这个消费相关的线程(和拉取消息的不一样)
     */
    ...
    public void pullMessage(final PullRequest pullRequest) {
        ...
        // 判断是否暂停中 消费者被挂起 将拉取任务延迟1秒 放入拉取任务队列 结束本次拉取
        if (this.isPause()) {
        ...
        // todo  进行消息拉取流控 从消息消费数量和消费间隔进行控制
        // 判断是否超过最大持有消息数量。默认最大值为1000。
        long cachedMessageCount = processQueue.getMsgCount().get();
        //
        long cachedMessageSizeInMiB = processQueue.getMsgSize().get() / (1024 * 1024);

        //超过pullThresholdForQueue 触发流控 放弃本次拉取任务
    ...
    //这个是什么？？
        ...
        int sysFlag = PullSysFlag.buildSysFlag(
            ...
            classFilter // class filter  消息过滤机制为类过滤器机制
        );
    ...
    public void resume() {
        this.pause = false;
        doRebalance();
        log.info("resumRebalanceServicee this consumer, {}", this.defaultMQPushConsumer.getConsumerGroup());
    }
    ...
    public synchronized void start() throws MQClientException {
        ...
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                //检查各参数是否正确 注意消费组名字不能是DEFAULT_CONSUMER
                this.checkConfig();
                ...
                if (this.defaultMQPushConsumer.getOffsetStore() != null) {
                    ...
                    //这里会根据不同的消费模式 创建不同的offsetStore对象
                ...
                // 注册Consumer 保证CID是单例模式
                //再次说明 在同一个JVM中所有的消费者 生产者持有同一个MQClientInstance 只会启动一次？
                ...
                //13、todo 启动MQClientInstance，会启动PullMessageService和RebalanceService
    ...
    /**
     * ，这里主要做两件事，第一是将订阅信息跟RebalanceImpl同步，这个类是consumer的核心逻辑实现类，
    ...
    private void copySubscription() throws MQClientException {
        try {
            ...
            //看代码 这里map这里sub的值不可能为空但是 map里面的size可以为空0
            System.out.println("========copySubscription=copySubscription================="+sub.size());
            if (sub != null) {
            ...
            switch (this.defaultMQPushConsumer.getMessageModel()) {
                case BROADCASTING:
                    break;
                case CLUSTERING:
                    //消费模式在集群模式下 订阅重试主题消息  并且这里可以看出消息重试是以消费组为单位 并不是主题
                    // 消息重试的主题名字为%RETRY%+消费组名 消费者启动时候自动订阅该主题 参与该主题消息队列的负载
    ...
    /**
     * PushConsumer 订阅topic
     * @param topic
     * @param subExpression
     * @throws MQClientException
     */
    public void subscribe(String topic, String subExpression) throws MQClientException {
        System.out.println("【消费者PushConsumer 订阅topic并且同步consumer信息到broker】");
        try {
            // 创建订阅数据
            SubscriptionData subscriptionData = FilterAPI.buildSubscriptionData(this.defaultMQPushConsumer.getConsumerGroup(),
                topic, subExpression);
